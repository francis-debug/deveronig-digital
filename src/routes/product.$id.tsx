...
export const Route = createFileRoute("/product/$id")({
  ...
  notFoundComponent: () => (
    <PageShell><p>Product not found.</p></PageShell>
  ),
  errorComponent: ({ error }) => (
    <PageShell><p>Something went wrong: {error.message}</p></PageShell>
  ),
...
function ProductPage() {
  ...
  return (
    <PageShell>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-md p-3 shadow-lg lg:hidden">
        <div className="mx-auto flex max-w-7xl items-center gap-3">
          <div className="flex flex-1 items-center gap-2">
            ...
            <div className="flex items-center rounded-full border border-border bg-secondary">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="px-2 py-1.5 hover:text-primary" aria-label="Decrease">
                <Minus className="h-4 w-4" />
              </button>
              <span className="min-w-6 text-center text-sm font-semibold tabular-nums">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="px-2 py-1.5 hover:text-primary" aria-label="Increase">
                <Plus className="h-4 w-4" />
              </button>
            </div>
          </div>
