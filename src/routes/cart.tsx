...
export const Route = createFileRoute("/cart")({
  head: () => ({ meta: [{ title: "Your Cart — Deveronig" }] }),
  component: CartPage,
});
    ...
    <PageShell>
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-3">
        <div className="space-y-3 lg:col-span-2">
          {items.map((it) => (
            <div key={it.id} className="flex gap-3 rounded-xl border border-border bg-card p-3 sm:gap-4 sm:p-4">
              <Link to="/product/$id" params={{ id: it.id }} className="h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-secondary sm:h-28 sm:w-28">
                <img src={it.image} alt={it.title} className="h-full w-full object-cover" />
              </Link>
              <div className="flex flex-1 flex-col">
                <Link to="/product/$id" params={{ id: it.id }} className="line-clamp-2 text-sm font-medium text-foreground hover:text-primary sm:text-base">
                  {it.title}
                </Link>
        ...
        <aside className="h-fit rounded-xl border border-border bg-card p-5">
          ...
          <Link to="/" className="mt-2 block text-center text-xs text-muted-foreground hover:text-primary">
            ← Continue Shopping
          </Link>
        </aside>
      </div>
